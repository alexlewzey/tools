#! /usr/bin/env python3
"""wrap the clipboard with a function specified as a cmd line arg (if fun from run window) or via user interface
is run from listener_standard script"""
import functools
import logging
import re
import textwrap
...
def line_at_caret_to_cb() -> str:
    """Regardless of cursor position select the whole line and copy it to the clipboard"""
...
def word_at_caret_to_cb() -> str:
    """Select word to the left of the cursor and copy to clipboard, Equivalent to shift + alt + left"""
...
def pad_right_full(char: str, left_len: int = 1) -> None:
    """
    add a left justify fill of hash characters to the current clipboard item to a length of 120 character and return it
    to the clipboard
...
def fmt_hash_center():
    """
    add a center justify fill of hash characters to the current clipboard item to a length of 120 character and return
    it to the clipboard
...
def unnest_parathesis():
    """extract the content of the paraenthesis from the current clipboard selection and type it out
...
def rm_doublespace() -> None:
    """copy selected text to clipboard format any consecutive spaces as a single space and add the return string to the
    clipboard
    """
...
def remove_consecutive_space(text: str) -> str:
    ...
    line_with_no_consecutive_spaces = [
...
def fmt_repr():
    """
    copy class properties to the clipboard, run this program and it will format the properties as a human readable repr
    string that you can add to your class
...
def test_valid_input(lines: List[str]) -> None:
    if not any(['.' in line for line in lines]):
        raise InvalidInputError(
            'Are you using the correct input? eg self.name = name')
...
def fmt_pycharm_params():
    """
    format pycharm params (from clipboard) as args and copies to clipboard. you can return multi-line params by passing
    in the command line arg nl
...
def fmt_list() -> None:
    """format the clipboard as a python style list where each line represents a list item"""
...
def fmt_as_multiple_lines() -> None:
    """transform a sequence separated by white space into a string where every item in the sequence is on a new line
...
def text2list(text: str) -> List[str]:
    """
    convert a multi-line segment of text and convert it into a list
    where every line represent a list item
    """
    return [x.strip(' ,\n') for x in text.split('\n') if x]
...
def fmt_class_properties_multiline():
    """format the arguments of a class as class properties to paste into the __init__ function of the class
...
def fmt_class_properties_multiassign():
    ...
    fmt_multiassign = lambda params: ', '.join([f'self.{param}' for param in params]) + ' = ' + ', '.join(
...
def get_class_properties(text: str) -> List[str]:
    if 'self' in text: text = text.split('self,')[-1]
